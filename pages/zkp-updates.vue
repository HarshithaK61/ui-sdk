    ...
    <UCard>
      <div class="container mx-auto p-4">
        <h2 class="text-xl font-bold mb-4">
          ZKP Verification - Scan Merchant QR
        </h2>
        ...
        <div class="mb-4">
          <label class="block text-sm font-medium mb-2"
            >Enter Merchant WalletConnect URI manually:</label
          >
        ...
        <div
          v-if="isScanning"
          class="mt-4 w-full max-w-md mx-auto qr-container"
        >
          <QrcodeStream
          ...
          <UCard>
            ...
            <div class="space-y-3">
              ...
              <div class="flex items-center gap-3">
                <div
                  class="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold bg-green-500 text-white"
                >
                  1
                </div>
              ...
              <div class="flex items-center gap-3">
                <div
                  :class="[
                    ...
                    [
                      ...
                      'generating-proof',
                      'proof-sent',
                    ].includes(zkpFlowStage)
                      ? 'bg-green-500 text-white'
                      : zkpFlowStage === 'merchant-scanned'
                ...
                >
                  {{
                    zkpFlowStage === "merchant-scanned"
                      ? "Pairing with Merchant..."
                      : "Paired with Merchant"
                  }}
                </span>
                <span
                  v-if="
              ...
              <div class="flex items-center gap-3">
                <div
                  :class="[
                    ...
                    [
                      ...
                      ? 'bg-green-500 text-white'
                      : zkpFlowStage === 'id-app-connecting'
                        ? 'bg-blue-500 text-white animate-pulse'
                        : 'bg-gray-300 text-gray-600',
                  ]"
                >
                ...
                >
                  {{
                    zkpFlowStage === "id-app-connecting"
                      ? "Connecting to ID App..."
                      : "ID App Session Approved"
                  }}
                </span>
                <span
                  v-if="
            ...
            <div v-if="zkpFlowStage === 'session-approved'" class="mt-4">
              <UAlert
                ...
                description="Use the ID App popup to generate your proof."
              />
            </div>
            ...
            <div v-if="zkpFlowStage === 'proof-sent'" class="mt-4">
              <UAlert
                color="green"
                title="Success!"
                description="Proof has been verified and sent to the merchant."
              />
              <UButton
                @click="resetZkpFlow"
                color="gray"
          ...
          <UCard>
            ...
            <template #footer>
              <div class="flex gap-2">
                ...
                <UButton @click="clearScannedUri" color="gray" size="sm">
                  Clear
                </UButton>
              </div>
            </template>
              ...
              >
                <div>
                  ...
                  <p class="text-xs text-gray-500">
                    Topic: {{ session.topic.slice(0, 20) }}...
                  </p>
                </div>
                <UButton
          ...
          <UCard>
            <template #header>
              <div class="flex justify-between items-center">
                ...
                <UButton @click="clearAllPairings" color="red" size="xs"
                  >Clear All</UButton
                >
              </div>
            </template>
            <div class="space-y-2">
              ...
              >
                <p class="text-sm font-medium">
                  {{ pairing.peerMetadata?.name || "Pending..." }}
                </p>
                <p class="text-xs text-gray-500">
                  Topic: {{ pairing.topic.slice(0, 20) }}...
                </p>
                <p
                  class="text-xs"
                  :class="pairing.active ? 'text-green-500' : 'text-yellow-500'"
                >
...
const generateProof = async () => {
  ...
  try {
    ...
    console.log(
      "[ID_APP] Merchant presentation request:",
    ...
    console.log(
      "[MERCHANT] Merchant session topic:",
...
const sendProofToMerchant = async (proof: any) => {
  ...
  console.log(
    "[MERCHANT] merchantSessionTopic.value:",
    merchantSessionTopic.value,
  );
  ...
  try {
    ...
    console.log(
      "[MERCHANT] About to call merchantSignClient.respond() with:",
    ...
    console.log(
      "[MERCHANT] merchantSignClient.respond() completed successfully!",
    );
...
const initMerchantSignClient = async () => {
  try {
    ...
    merchantSignClient.on("session_request", async (event: any) => {
      console.log(
        "[MERCHANT] *** SESSION REQUEST RECEIVED FROM MERCHANT ***",
        event,
      );
      ...
      if (
        request.method === "request_verifiable_presentation_v1" ||
        request.method === "concordium_request_verifiable_presentation"
      ) {
        zkpFlowStage.value = "request-received";
        ...
        console.log(
          "[MERCHANT] Stored presentation request:",
          merchantPresentationRequest.value,
        );
        console.log(
          "[MERCHANT] Stored session topic:",
          merchantSessionTopic.value,
        );
        console.log("[MERCHANT] Stored request ID:", merchantRequestId.value);

        setStatus(
          "Presentation request received! Connecting to ID App...",
          "green",
        );
      ...
      try {
        ...
        if (requiredNamespaces) {
          for (const [key, value] of Object.entries(
            requiredNamespaces as Record<string, any>,
          )) {
            namespaces[key] = {
              accounts:
                value.chains?.map(
                  (chain: string) => `${chain}:dummy_account`,
                ) || [],
        ...
        if (optionalNamespaces) {
          for (const [key, value] of Object.entries(
            optionalNamespaces as Record<string, any>,
          )) {
            if (!namespaces[key]) {
              namespaces[key] = {
                accounts:
        ...
        const approvedSession = merchantSignClient!.session.get(
          ackResult?.topic,
        );
        console.log("[MERCHANT] Approved session details:", approvedSession);
        console.log(
          "[MERCHANT] Session namespaces:",
          approvedSession?.namespaces,
        );

        zkpFlowStage.value = "merchant-paired";
        setStatus(
          "Paired with merchant! Waiting for presentation request...",
          "green",
        );
        updateMerchantSessions();
...
const pairWallet = async () => {
  ...
  try {
    ...
    const pairingTopic = await merchantSignClient.pair({
      uri: scannedUri.value,
    });
  ...
  try {
    ...
    for (const session of sessions) {
      try {
        ...
        console.log(
          `[MERCHANT] Failed to disconnect session ${session.topic}:`,
          e,
        );
      }
    }
    ...
    for (const pairing of pairings) {
      try {
        await merchantSignClient.core.pairing.disconnect({
          topic: pairing.topic,
        });
      } catch (e) {
